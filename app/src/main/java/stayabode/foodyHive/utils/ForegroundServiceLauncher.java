...
public class ForegroundServiceLauncher {
    ...
    public synchronized void startService(Context context)
    {
        ...
        if (!isServiceRunning(context))
        {
            if (isOreoOrHigher())
            {
                context.startForegroundService(new Intent(context, ServiceForRefreshToken.class));
            }
            else
            {
                context.startService(new Intent(context,ServiceForRefreshToken.class));
            }
        }
    ...
    {
        context.stopService(new Intent(context, ServiceForRefreshToken.class));
    }
    ...
    private boolean isServiceRunning(Context ctx) {
        ...
        for (ActivityManager.RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
            if (ServiceForRefreshToken.class.getName().equals(service.service.getClassName())) {
